			...
			<div class="shell">
				<div class="range offset-md-top-90 offset-top-0">
					...
					<div class="cell-md-4">
						...
						<p class="offset-top-25">Размер памятника: 100х60х5 см</p>
					...
					<!-- Responsive-tabs-->
						<div class="responsive-tabs responsive-tabs-default offset-top-40 responsive-tabs-default-variant-2 text-left">
							...
							<div class="resp-tabs-container resp-tabs-container-2">
								...
								<div>
<?php  if(empty($data['comment']['primary']) && empty($data['comment']['user'])){?>
...
<?php } else {?>
	<?php foreach($data['comment']['primary'] as $key => $value){?>
		<?php if($data['comment']['primary'][$key]['level'] == 1){?>
									<div class="media media-variant-3 text-center text-sm-left">
										<div class="media-left round">
											<img src="images/post-page-4-70x70.jpg" alt="" width="70" height="70">
										</div>
										<div class="media-body">
											<h5 class="txt-matrix"><?php echo $data['comment']['user'][$key]['name']?> <?php echo $data['comment']['user'][$key]['subname']?></h5>
											<h6><?php echo $data['comment']['primary'][$key]['create_at']?></h6>
											<p class="offset-md-top-20"><?php echo $data['comment']['primary'][$key]['comment']?></p><a href="#" class="icon icon-matrix fa-mail-reply icon-sm-2"></a>
										</div>
									</div>
		<?php } else if($data['comment']['primary'][$key]['level'] == 2){?>
									<div class="media media-variant-3 offset-top-23 inset-md-left-102 text-center text-sm-left">
										...
										<div class="media-body">
											<h5 class="txt-matrix"><?php echo $data['comment']['user'][$key]['name']?> <?php echo $data['comment']['user'][$key]['subname']?></h5>
											<h6><?php echo $data['comment']['primary'][$key]['create_at']?>
												<br class="veil-sm">
												<span class="icon icon-darker fa-mail-reply icon-xxs-2"></span>
											</h6>
											<p class="offset-md-top-20"><?php echo $data['comment']['primary'][$key]['comment']?></p>
