...
/*
foreach ($array['figure'] as $key => $value){
	
	$key++;
	
	$figure[$key] = $value['name'];
	//$figure[$key]['img'] = $value['img'];
			...
			<div class="shell-wide text-center text-md-center">
				...
				<div class="responsive-tabs responsive-tabs-default offset-top-40 responsive-tabs-btn text-center">
					...
					<div class="text-center text-md-center">
						<button class="btn btn-transparent-2 btn-xs">Обновить стоимость</button><h4>0 руб.</h4>
					</div>
					<style>
						.btn-constructor{
							display: flex;
					...
					<div class="resp-tabs-container">
						<div>
							<div class="range">
								<div class="cell-md-5">
								<?= $form->field($model, 'id_stone', ['template'=>'{label}{input}{hint}{error}'])
								->dropDownList(
									$stone, $params['stone']
								)
								->label('Тип камня:');?>
								
								<?= $form->field($model, 'id_size', ['template'=>'{label}{input}{hint}{error}'])
								->dropDownList(
									$size, $params['size']
								)
								->label('Размер памятника:');?>
								
								<?= $form->field($model, 'id_figure')
								->radioList(
									$figure_label
								,[
									...
									function ($figure_index, $figure_label, $figure_name){
								...
								<?= Html::submitButton('Проверка', ['class' => 'btn btn-primary btn-sm btn-min-width-230', 'name' => 'contact-button']) ?>
								</div>
